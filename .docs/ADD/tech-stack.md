...
| Schema migrations | files in `supabase/migrations/` (CLI naming, `qbo_` names) = all DDL / init code; applied via Supabase MCP `apply_migration` after USER approval; never `supabase db push` on `vosk.dev` | decided (apply path proposed) | ADR-0001, ADR-0002, ADR-0003, ADR-0008 |
...
| Schema layout + Data API exposure + DB roles | one schema `qbo` (all layers); roles `qbo_etl` (ETL + dbt) and `qbo_reader` (Power BI, `vw_*` only); `qbo` not exposed through the Data API; web path with ADR-0005 | decided (schema) · proposed (roles, exposure) | ADR-0003 |
...
| OAuth token vault | Supabase Vault secret `qbo_refresh_token` behind two `security definer` functions (USER said "Supabase env vars"; interpretation to confirm) | proposed | ADR-0007 |
